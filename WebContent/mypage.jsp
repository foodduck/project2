		...
		<%@include file='header.jsp' %>
		 ...
		 <div class="content fix">
            <div class="breadcrumb">
                ...
                <div class="sub_main">
            		<form action="UpdateMypageCtrl" method="post" name="joinform" onsubmit="return joinCheck(this)" >
            			<input type="hidden" name="usertbid" value="${usertb.getUsertbid() }" >
            			<input type="hidden" name="cp" value="${usertb.getCp() }" >            			
						<table class = "table">
							<tr>
								<th class="item1">아이디</th>
								<td class="item1"><input type="text" name="u_id" value="${usertb.getU_id() }" required autofocus ></td>
								<td>
							...
							<tr>
								...
								<td class="item3"><input type="text" name="uname" value="${usertb.getUname() }" required ></td>
							</tr>
							<tr>
								<th class="item4">이메일</th>
								<td class="item4"><input type="email" name="email" value="${usertb.getEmail() }" ></td>
							</tr>
							<tr>
								<th class="item5">질문</th>
								<td class="item5"><input type="text" name="q" required value="${usertb.getQ() }" ></td>
							</tr>
							<tr>
								<th class="item6">답</th>
								<td class="item6"><input type="text" name="a" required value="${usertb.getA() }" ></td>
							</tr>
							<tr>
								<th class="item7">우편번호</th>
								<td class="item7"><input type="text" name="add1" id="add1" value="${usertb.getAdd1() }"></td>
								<td><input type="button" value="주소찾기" onclick="findAddr()" class="submit_btn" /></td>	
							</tr>
								<tr>
								<th class="item7">주소</th>
								<td class="item7"><input type="text" name="add2" id="add2" value="${usertb.getAdd2() }"></td>
							</tr>
						</table>
						<button type="submit" class="submit_btn" style="margin-left:230px;">수정완료</button>
					</form>	
					<a href="DeleteUsertbCtrl?usertbid=${usertb.getUsertbid() }" class="submit_btn" id="delete_btn">회원탈퇴</a>
            	</div>
